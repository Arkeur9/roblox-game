...
local KNOWN_EVENTS = {
	"NotifySuccess", "NotifyError", "BroadcastAnnouncement", "NotifyBossRewards",
	"SyncActivePotions", "PlayRollVisuals", "RequestRoll", "ToggleEquipCard",
	"RequestUpgrade", "SyncPartnerOffer", "PartnerAcceptedState", "EnterDoubleConfirm",
	"TradeCompleted", "TradeCancelled", "UpdateTradeOffer", "AcceptTradeOffer",
	"ConfirmTradeFinal", "CancelTrade", "SyncData", "UpdateLeaderboardCache"
}

local KNOWN_FUNCTIONS = {
	"GetProfile", "RequestUsePotion", "RequestCraft", "RequestFusion",
	"RequestTeleportToWorld", "RequestUnlockWorld", "RequestCodeRedeem",
	"SendTradeRequest", "AcceptTradeInvite", "GetLeaderboard"
}
...
local function getRemotesFolder()
	...
	if RunService:IsServer() then
		...
		-- Pre-create known RemoteFunctions
		for _, name in ipairs(KNOWN_FUNCTIONS) do
			if not remotesFolder:FindFirstChild(name) then
