...
local function refreshInventoryPanel(scroller)
	...
	if inventorySubTab == "Cards" then
		...
		for _, item in ipairs(localData.Inventory) do
			...
			rarityLbl.Font = Enum.Font.SourceSans
...
local function refreshUpgradesPanel(scroller)
	...
	for _, upgrade in ipairs(upgradeTypes) do
		...
		lvlLbl.Font = Enum.Font.SourceSans
		lvlLbl.TextSize = 12
...
local function refreshCraftingPanel(scroller)
	...
	for recKey, rInfo in pairs(CraftingConfig.Recipes) do
		...
		reqsLbl.Font = Enum.Font.SourceSans
		reqsLbl.TextSize = 10
...
local function refreshWorldsPanel(scroller)
	...
	for _, wKey in ipairs(WorldsConfig.Order) do
		...
		multLbl.Font = Enum.Font.SourceSans
		multLbl.TextSize = 12
...
local function refreshQuestsPanel(scroller)
	...
	local renderQuest = function(category, key, info)
		...
		progLbl.Font = Enum.Font.SourceSans
		progLbl.TextSize = 10
...
function UIController.Start()
	-- ① Créer l'UI EN PREMIER, sans aucun appel réseau bloquant
	UIController.CreateGUI()
	print("[UIController] GUI créée avec succès.")

	-- ② Connexion aux events réseau de façon NON-BLOQUANTE
	task.spawn(function()
		-- Attendre que le dossier Remotes existe (serveur prêt)
		local RS = game:GetService("ReplicatedStorage")
		local remotesFolder = RS:WaitForChild("Remotes", 30)
		if not remotesFolder then
			warn("[UIController] Timeout: dossier Remotes introuvable après 30s.")
			return
		end

		-- Récupérer les données initiales du profil
		local getProfileFunc = remotesFolder:WaitForChild("GetProfile", 10)
		if getProfileFunc then
			...
			if ok and data then
				...
				print("[UIController] Données profil chargées.")
			else
				warn("[UIController] Impossible de charger le profil: " .. tostring(data))
			end
		else
			warn("[UIController] RemoteFunction GetProfile introuvable.")
		end

		-- Écouter les synchronisations en temps réel
		local syncRemote = remotesFolder:WaitForChild("SyncData", 10)
		if syncRemote then
			syncRemote.OnClientEvent:Connect(function(newData)
				localData = newData
				UIController.UpdateHUD()

				-- Rafraîchir le panel actif si ouvert
				if activeTab then
			...
			print("[UIController] SyncData connecté.")
		else
			warn("[UIController] RemoteEvent SyncData introuvable.")
		end
	end)
end
